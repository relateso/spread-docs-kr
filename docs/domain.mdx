...
![도메인 추가 후 보이는 DNS 레코드 예시](/images/domain/dns-records-example.png)
...
아래 방법에 따라 envelope 서브 도메인 MX·SPF 레코드를 추가해주세요.

### MX 레코드 추가하는 방법

- 호스트 : envelope.domain (ex. envelope.relate.so)
- 타입 : MX
- 값 / 우선순위 : 스프레드 도메인 설정 페이지에서 제공하는 값과 우선순위를 그대로 입력합니다.

<Warning>
**가비아에서는 MX 값(메일 서버 주소) 끝에 마침표(`.`)를 반드시 붙여야 합니다.** 가비아는 마침표를 자동으로 추가하지 않습니다. 예를 들어 제공된 값이 `...amazonses.com` 으로 끝난다면 `...amazonses.com.` 처럼 **맨 끝에 마침표를 포함**해 입력하세요.

마침표를 빠뜨리면 값 뒤에 도메인(존) 이름이 자동으로 붙어 `...amazonses.com.relate.so` 같은 잘못된 주소로 저장되고, 메일 수신이 되지 않습니다.
</Warning>

<Note>
우선순위는 값 칸이 아니라 **별도의 우선순위 칸에 숫자만** 입력하세요. (값 칸에 `10 ...`처럼 우선순위를 함께 쓰지 않습니다.) 등록 후 터미널에서 `dig MX 도메인` 을 실행하거나 [MXToolbox](https://mxtoolbox.com/)에서 실제 반영된 값을 확인할 수 있습니다. 마침표를 빠뜨린 경우 조회 결과에 도메인이 두 번 겹쳐(`...amazonses.com.relate.so`) 표시되므로 바로 확인됩니다.
</Note>
...
카페24에서 구매한 도메인은 카페24 DNS 관리 페이지에서 레코드를 직접 추가할 수 있습니다.

### DNS 관리 메뉴 위치

- **쇼핑몰 호스팅**: 쇼핑몰 관리자 → **쇼핑몰 설정 > 기본 설정 > 도메인 설정 > 도메인 관리**에서 해당 도메인의 `DNS 설정`으로 이동합니다.
- **일반 호스팅**: **나의 서비스 관리 > 도메인 관리 > DNS 관리**로 이동합니다.

DNS 관리 페이지에서 `레코드 추가` 버튼을 눌러 스프레드에서 제공하는 MX, TXT, CNAME 값을 하나씩 추가합니다.

<Note>
메인 도메인의 기존 메일(네이버/지메일 등) 설정에 영향을 주지 않으려면, 기존 레코드를 수정하지 말고 스프레드가 제공하는 envelope 서브도메인 레코드를 그대로 추가해주세요.
</Note>

### MX 레코드 추가하는 방법

- 호스트(이름) : envelope.domain (ex. envelope.relate.so)
- 타입 : MX
- 값 / 우선순위 : 스프레드에서 제공하는 값과 우선순위를 그대로 입력

### SPF · DMARC 레코드 추가하는 방법

- 타입 : TXT
- 호스트(이름) / 값 : 스프레드 도메인 설정 페이지의 Name·Value 값을 각각 복사해서 붙여 넣습니다.
- SPF 예 : v=spf1 include:amazonses.com ~all (""는 자동으로 붙으니 추가하지 않으셔도 됩니다.)

### DKIM(CNAME) 레코드 추가하는 방법

- 타입 : CNAME
- 호스트(이름) / 값 : 스프레드에서 제공하는 도메인키 Name·Value를 그대로 입력합니다.

<Info>
레코드는 도메인당 MX 5개, CNAME 10개, TXT 10개까지 추가할 수 있으며, 변경 사항이 적용되기까지 약 30분~1시간이 소요될 수 있습니다.
</Info>

더 자세한 내용은 카페24 공식 가이드를 참고해주세요: [카페24 DNS 설정 가이드](https://support.cafe24.com/hc/ko/articles/7671674036249-DNS-%EC%84%A4%EC%A0%95)
